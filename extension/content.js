...
async function fetchLyrics() {
  try {
    ...
    if (!isYouTubeVideoPage()) {
      content.innerHTML = '<div class="error">This extension only works on YouTube video pages.</div>';
      return;
    }
    
    // Show loading state
    content.innerHTML = '<div class="loading">Loading lyrics...</div>';
    ...
    if (!title) {
      content.innerHTML = '<div class="error">Could not extract video title</div>';
      return;
    }
    ...
    if (data.success && data.result) {
      ...
      content.innerHTML = `<div class="error">No lyrics found for "${title}"</div>`;
    }
  } catch (error) {
    content.innerHTML = `<div class="error">Error fetching lyrics: ${error.message}</div>`;
...
function displayLyrics(result) {
  ...
  try {
    ...
    // Create HTML for the lyrics display - removed source information
    const lyricsHTML = `
